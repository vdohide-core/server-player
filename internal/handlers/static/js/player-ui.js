...
function initCustomUI(playerId, playerInstance) {
    ...
    const openScreenshotModal = () => {
        ...
        // ออก fullscreen ก่อน (ถ้ามี) + delay ให้ menu หายก่อนแคป
        const startCapture = () => setTimeout(doCapture, 100);
        ...
        if (fsEl) {
            ...
            if (exitFn) { exitFn(); setTimeout(doCapture, 500); } else { startCapture(); }
        } else { startCapture(); }
        ...
        if (!window.html2canvas) {
            ...
            script.onload = () => {
                const fsEl2 = document.fullscreenElement || document.webkitFullscreenElement;
                if (fsEl2) { const fn = document.exitFullscreen ? document.exitFullscreen.bind(document) : document.webkitExitFullscreen ? document.webkitExitFullscreen.bind(document) : null; if (fn) { fn(); setTimeout(doCapture, 400); } else doCapture(); }
                else doCapture();
            };
    ...
    const setupResumePlayback = () => {
        const cfg = window.PLAYER_CONFIG || {};
        const vodId = cfg.vodId;
        const resumeMode = cfg.resumePlay; // "ask" | "auto" | "off" | false
    ...
    const openSettingsModal = () => {
        ...
        const formatQualityLabel = (lv) => {
            if (!lv) return '';
            // ตรวจสอบจาก height เป็นหลัก (ตามที่ Request: 2160 = 2K, 4320 = 4k)
            if (lv.height) {
                const h = parseInt(lv.height);
                if (h === 4320) return '4K';
                if (h === 2160) return '2K';
            ...
            // ถ้าไม่มี height ตรวจจาก string label (Fallback)
            let lbl = lv.label || '';
            if (lbl.includes('4320')) return '4K';
            if (lbl.includes('2160')) return '2K';
            return lbl;
        };
...
${hasMultiCaptions ? `<div id="sp-view-caption" style="display:none;flex-direction:column;overflow:hidden;">
  ...
  <div id="sp-caption-list" style="overflow-y:auto;">${captionHTML}</div>
    ...
    setupTools({position:"bottom-right", button:["pip","fullscreen"]});
