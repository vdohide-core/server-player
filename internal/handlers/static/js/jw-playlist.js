/**
 * JW Player embed — loads media via playlist.json feed from server-player.
 * Requires playerConfig.slug — media/ad details come from /playlist/{slug}.json
 */
...
class PlayerSetup {
  ...
  async init() {
    ...
    // Advert image overlay — init after player DOM is ready
  ...
  async createConfig() {
    ...
    const feedUrl = config.slug
      ? `${window.location.origin}/playlist/${encodeURIComponent(config.slug)}.json`
      : null;

    if (feedUrl) {
      baseConfig.playlist = feedUrl;
